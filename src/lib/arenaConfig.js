...
export const BADGE_EFFECTS = {
  // ── COMMON ──
  divisible_2:      { name: 'Swift Stride',     desc: '+8% move speed',                    mods: { speed: 1 } },
  divisible_3:      { name: 'Triple Tap',       desc: '+2 attack damage',                  mods: { damage: 0 } },
  divisible_5:      { name: 'Fiver Frenzy',     desc: '+8% attack speed',                  mods: { attackSpeed: 1 } },
  divisible_7:      { name: 'Lucky Seven',      desc: '+6% crit chance',                  mods: { crit: 0 } },
  contains_zero:    { name: 'Zero Gravity',      desc: 'Attacks slow enemies by 20%',       flags: [] },
  ends_zero:        { name: 'Round Hide',       desc: '+15 max HP',                        mods: { maxHp: 0 } },
  hydrogen:         { name: 'Atomic Base',       desc: '+20 max HP',                        mods: { maxHp: 0 } },
  evil:             { name: 'Corruption',        desc: 'Attacks deal 3% lifesteal',         mods: { lifesteal: 0 } },
  deficient:        { name: 'Deficient Guard',  desc: 'Take 6% less damage',               mods: { defense: 0 } },

  // ── UNCOMMON ──
  meme_69:          { name: 'Nice Executioner', desc: '+20% damage to enemies below 30% HP', flags: ['execute'] },
  meme_420:         { name: 'Blaze It',          desc: 'Regen 1.5 HP/sec',                  mods: { regen: 0.1875 } },
  meme_1337:        { name: 'Leet Hands',       desc: '+14% attack speed',                 mods: { attackSpeed: 1.0175 } },
  meme_8008:        { name: 'Bouncy',            desc: '+20 max HP, bounce attacks',        mods: { maxHp: 2.5 }, flags: ['bounce'] },
  meme_69420:       { name: 'Ultimate Meme',    desc: '+8% to all stats',                  mods: { speed: 1.01, damage: 0.5, attackSpeed: 1.01, maxHp: 1.875 } },
  divisible_100:    { name: 'Centurion',        desc: '+30 max HP',                        mods: { maxHp: 3.75 } },
  ...
  boron:            { name: 'Boron Plating',    desc: '+12% defense',                      mods: { defense: 0.015 } },
  carbon:           { name: 'Carbon Edge',      desc: '+12% attack damage',                mods: { damage: 0.5 } },
  oxygen:           { name: 'Oxygen Tank',      desc: 'Regen 2.5 HP/sec',                  mods: { regen: 0.3125 } },
  happy:            { name: 'Happy Shield',     desc: '8% chance to negate incoming damage', flags: ['happyDodge'] },
  harshad:          { name: 'Harshad Speed',    desc: '+12% move speed',                   mods: { speed: 1.015 } },
  abundant:         { name: 'Abundant Vitality',desc: '+40 max HP',                        mods: { maxHp: 5 } },
  pronic:           { name: 'Pronic Jump',       desc: 'Every 3rd attack deals 2x damage',  flags: ['pronicCombo'] },

  // ── RARE ──
  prime:            { name: 'Prime Critical',   desc: 'Crits deal 2.5x damage',            mods: { critDmg: 1.7 } },
  perfect_square:   { name: 'Square Guard',     desc: '+20% defense',                      mods: { defense: 0.04 } },
  ...
  triple_six:       { name: 'Devil\'s Bargain', desc: '+30% damage, -15 max HP',           mods: { damage: 1.6, maxHp: -3 }, quirky: true },
  repeated_digit:   { name: 'Echo Strike',      desc: 'Every attack hits twice',           flags: ['doubleHit'] },
  gold:             { name: 'Golden Aura',      desc: '+15% to all stats',                 mods: { speed: 1.03, damage: 1.2, attackSpeed: 1.03, maxHp: 5 } },
  triangular:       { name: 'Triangular Slash', desc: 'Attacks hit in a triangular AoE',   flags: ['triangularAoE'] },
  smith:            { name: 'Smith Armor',      desc: '+22% defense',                      mods: { defense: 0.044 } },
  lucas:            { name: 'Lucas Precision',  desc: '+18% crit chance',                  mods: { crit: 0.036 } },
  ...
  // ── EPIC ──
  perfect_cube:     { name: 'Cubed Crits',      desc: 'Crits deal 3x damage',              mods: { critDmg: 1.9125 } },
  ...
  six_nines:        { name: 'Nine Inferno',      desc: '+999% damage (capped at +50%)',     mods: { damage: 4.875 } },
  deep_void:        { name: 'Void Pull',         desc: 'A black hole pulls enemies toward you', flags: ['voidPull'], quirky: true },

  // ── MYTHIC ──
  one_million:      { name: 'Million Strike',   desc: '+40 attack damage',                 mods: { damage: 15 } },
  ...
  golden_ratio:     { name: 'Golden Form',       desc: '1.618x all stats',                  mods: { speed: 1.232, damage: 2.25, attackSpeed: 1.1125, maxHp: 11.25, crit: 0.06 } },
  euler:            { name: 'Exponential Growth',desc: 'Damage grows by e (2.718x) over 10s', flags: ['exponential'], quirky: true },

  // ── RAINBOW ──
  perfect_number:   { name: 'Perfection',       desc: 'Immune to damage for 3s, +50 all stats', flags: ['perfectImmune'], mods: { damage: 20, maxHp: 20, speed: 1.12 } },
  ...
  // ── 67 SERIES ──
  '67_i':           { name: '67% Lifesteal',     desc: 'Heal for 67% of damage dealt',      mods: { lifesteal: 0.536 } },
  ...
  // ── NEW ADDITIONS ──
  // COMMON
  square_root:      { name: 'Root Focus',       desc: '+7% crit chance, +5% attack speed', mods: { crit: 0, attackSpeed: 1 } },
  even_digits:      { name: 'Even Footing',     desc: '+10 max HP, +4% move speed',        mods: { maxHp: 1.25, speed: 1.005 } },
  odd_digits:       { name: "Odd One's Edge",   desc: '+3 attack damage',                  mods: { damage: 0.375 } },
  // UNCOMMON
  lucky_number:     { name: 'Fourleaf Fortune',  desc: '+10% crit chance, +10% crit damage', mods: { crit: 0, critDmg: 1.5 } },
  vampire_number:   { name: 'Vampiric Split',   desc: 'Attacks lifesteal 8%, deal 2 less damage', mods: { lifesteal: 0, damage: 0 } },
  half_balance:     { name: 'Balanced Halves',  desc: '+10% attack damage, +10% max HP',   mods: { damage: 0.375, maxHp: 1.875 } },
  // RARE
  twin_prime:       { name: 'Twin Strike',      desc: 'Fire 2 projectiles per attack',     flags: ['multishot3'] },
  ricochet:         { name: 'Ricochet Rounds',  desc: 'Projectiles bounce off arena walls', flags: [] },
  glass_cannon:      { name: 'Glass Cannon',      desc: '+35% attack damage, -20% defense',  mods: { damage: 2.75, defense: -0.055 } },
  // EPIC
  fortress:         { name: 'Fortress Stance',  desc: '+30% defense, -10% move speed',     mods: { defense: 0.0375, speed: 0.9875 } },
  berserker:        { name: "Berserker's Rage", desc: '+18% attack speed, +18% damage, -15% defense', mods: { attackSpeed: 1.0225, damage: 0.625, defense: -0.0185 } },
  phoenix:          { name: 'Phoenix Regen',    desc: 'Regen 4 HP/sec, +25 max HP',        mods: { regen: 0.5, maxHp: 3.125 } },
  // ANOMALY
  quintuple:        { name: 'Quintuple Threat', desc: 'Fire 5 projectiles in a spread',    flags: ['multishot5'] },
  overclock:        { name: 'Overclocked Core', desc: '+22% attack speed, +22% move speed', mods: { attackSpeed: 1.0605, speed: 1.0605 } },
};
