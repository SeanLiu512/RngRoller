...
export const db = {
  auth: {
    ...
    async verify2FALogin(pendingToken, code) {
      const result = await request('/api/auth/2fa/login-verify', { method: 'POST', body: { pendingToken, code } });
      if (result?.access_token) setToken(result.access_token);
      return result;
    },
    async setup2FA() {
    ...
    async verify2FASetup(code) {
      return request('/api/auth/2fa/verify-setup', { method: 'POST', body: { code } });
    },
    async disable2FA(password) {
      return request('/api/auth/2fa/disable', { method: 'POST', body: { password } });
    },
  ...
  game: {
    roll() { return request('/api/game/roll', { method: 'POST', body: {} }); },
    purchase(itemId) { return request('/api/game/purchase', { method: 'POST', body: { itemId } }); },
  },
  entities: {
